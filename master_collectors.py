...
- 개선: 지연 로딩, 설정 타입 검증, 로그 자동 정리, 드라이 런 모드, 중복 코드 제거
"""
import os
import sys
import json
    ...
    for key, typ in [('max_workers', int), ('cpu_factor', (int, float)), 
        ...
        if key in pconf:
            if not isinstance(pconf[key], typ):
                logger.error(f"Collector #{idx} parallel_config.{key} 타입 오류 (기대: {typ})")
                return False
