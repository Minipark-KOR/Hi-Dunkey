...
class AddressFilter:
    REGION_SHORTEN_PREFIX = [
        (r"^서울특별시\s*", "서울 "), (r"^부산광역시\s*", "부산 "),
        (r"^대구광역시\s*", "대구 "), (r"^인천광역시\s*", "인천 "),
        (r"^광주광역시\s*", "광주 "), (r"^대전광역시\s*", "대전 "),
        (r"^울산광역시\s*", "울산 "), (r"^세종특별자치시\s*", "세종 "),
        (r"^경기도\s*", "경기 "), (r"^강원도\s*", "강원 "),
        (r"^충청북도\s*", "충북 "), (r"^충청남도\s*", "충남 "),
        (r"^전라북도\s*", "전북 "), (r"^전라남도\s*", "전남 "),
        (r"^경상북도\s*", "경북 "), (r"^경상남도\s*", "경남 "),
        (r"^제주특별자치도\s*", "제주 "),
    ]
    ...
    ADMIN_DISTRICT_MAP = {
        "일광면": "일광읍", "산동면": "산동읍", "양북면": "문무대왕면",
        "퇴계원면": "퇴계원읍", "이동면": "이동읍", "비봉면": "비봉읍",
    }
    ...
    def advanced_clean(address: str) -> str:
        ...
        addr = re.sub(r'\([^)]*\)', '', addr)
        addr = re.sub(r'\[[^\]]*\]', '', addr)  # ✅ 대괄호 제거 패턴 수정됨
    ...
    def classify(address: str) -> str:
        ...
        if is_road and not is_jibun: return "road"
        if is_jibun and not is_road: return "jibun"
        if is_road and is_jibun: return "mixed"
        return "unknown"
