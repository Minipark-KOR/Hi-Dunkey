...
class CollectLog:
    ...
    def _init_db(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """)
            # 기존 DB와 호환: 컬럼이 없으면 추가
            cols = {
                row[1] for row in conn.execute("PRAGMA table_info(collect_run)").fetchall()
            }
            if "updated_at" not in cols:
                conn.execute("ALTER TABLE collect_run ADD COLUMN updated_at TEXT")
            conn.execute("""
    ...
    def start_run(self, step: str, region: str, shard: Optional[str] = None) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cur = conn.execute("""
                INSERT INTO collect_run (step, region, shard, started_at, updated_at, status)
                VALUES (?, ?, ?, ?, ?, 'running')
            """, (step, region, shard, now, now))
            return cur.lastrowid

    def finish_run(self, run_id: int, status: str, rows_saved: int = 0,
                   rows_before: int = 0, error_msg: str = "") -> None:
        """실행 완료 기록"""
        now = now_kst().isoformat()
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                UPDATE collect_run
                SET finished_at = ?, updated_at = ?, status = ?, rows_saved = ?, rows_before = ?, error_msg = ?
                WHERE run_id = ?
            """, (now, now, status, rows_saved, rows_before, error_msg, run_id))

    def get_failed_runs(self, step: Optional[str] = None) -> List[Dict[str, Any]]:
        """실패한 실행 목록 조회 (재시도용)"""
        with sqlite3.connect(self.db_path) as conn:
            ...
            if step:
                query += " AND step = ?"
                params.append(step)
            query += " ORDER BY COALESCE(updated_at, started_at) DESC, started_at DESC"
            cur = conn.execute(query, params)
            return [dict(row) for row in cur.fetchall()]

    def get_stats(self, step: str, date: str) -> Dict[str, int]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cur = conn.execute("""
                SELECT 
                    ...
                    COALESCE(SUM(rows_saved), 0) as total_rows
                FROM collect_run
