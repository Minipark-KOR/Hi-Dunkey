...
class RetryManager:
        ...
        with get_db_connection(self.db_path, timeout=30) as conn:
            conn.execute("""
                CREATE TABLE IF NOT EXISTS failures (
                    ...
                    last_error TEXT,
                    UNIQUE(domain, task_type, shard, sc_code, region, year, month, day, semester, sub_key)
                )
            """)
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_next_retry ON failures(next_retry_at) WHERE resolved=0"
            )
            conn.execute("CREATE INDEX IF NOT EXISTS idx_domain ON failures(domain)")
        ...
        with get_db_connection(self.db_path, timeout=30) as conn:
            try:
                cur = conn.execute("""
                    ...
                    WHERE domain=? AND task_type=? AND shard=? AND sc_code=? AND region=?
                      AND year=? AND month=? AND day=? AND semester=? AND sub_key=?
                """, (domain, task_type, shard, sc_code, region, year, month, day, semester, sub_key))
                row = cur.fetchone()
                ...
                if row:
                    ...
                    if resolved == 1:
                        next_time_str = (now_kst() + timedelta(minutes=1)).isoformat()
                        conn.execute("""
                            UPDATE failures SET
                            ...
                            WHERE id=?
                        """, (now_kst().isoformat(), next_time_str, error, max_retries, failure_id))
                        logger.info(f"포기된 작업 재활성화: {domain}/{task_type} id={failure_id}")
                        return True

                    retries += 1
                    if retries >= stored_max:
                        conn.execute("""
                            UPDATE failures SET resolved=1, resolution_status='GIVE_UP',
                                last_error=?, retries=? WHERE id=?
                        """, (error, retries, failure_id))
                        logger.warning(f"최대 재시도 초과 → 포기: {domain}/{task_type} sc_code={sc_code}")
                        return False

                    minutes = min(2 ** retries, 24 * 60)
                    next_time_str = (now_kst() + timedelta(minutes=minutes)).isoformat()
                    conn.execute("""
                        UPDATE failures SET retries=?, next_retry_at=?, last_error=?, resolved=0
                        WHERE id=?
                    ...
                    conn.execute("""
                        ...
                        (domain, task_type, shard, sc_code, region, year, month, day, semester,
                         address, sub_key, failed_at, retries, max_retries, next_retry_at, last_error)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        domain, task_type, shard,
                        sc_code, region, year, month, day, semester, address, sub_key,
                        now_kst().isoformat(),
    ...
    def get_pending_retries(
        ...
        with get_db_connection(self.db_path, timeout=30, row_factory=sqlite3.Row) as conn:
            query = "SELECT * FROM failures WHERE resolved=0 AND next_retry_at <= ?"
            params: list = [now_str]
            if domain:
                query += " AND domain=?"
    ...
    def mark_orphan(self, failure_id: int, error: str = ""):
        with get_db_connection(self.db_path, timeout=30) as conn:
            conn.execute("""
                UPDATE failures SET resolved=1, resolution_status='ORPHAN',
                    last_error=? WHERE id=?
            """, (error, failure_id))
        send_alert(
            f"데이터 정합성 오류 발생 (failure_id={failure_id}): 대상 레코드 없음",
            level="critical"
        )
