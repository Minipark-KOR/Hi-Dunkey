...
class RetryManager:
    ...
    def get_pending_retries(
        ...
        now = self._now()
        # cutoff = now (deadline은 스케줄링 시에만 사용)
        cutoff = now
    ...
    # core/retry.py (_compute_next_attempt 메서드 수정)

    def _compute_next_attempt(
        self, now: datetime, retries: int, deadline: Optional[datetime]
    ) -> Optional[datetime]:
        # ✅ retries 는 현재까지의 누적 시도 횟수 (첫 실패=1)
        # max_retries=3 이면 3 회까지 재시도 허용 (retries=1,2,3)
    ...
    # core/retry.py (record_failure 메서드 수정 부분)
    ...
    def record_failure(
        ...
        with self.get_connection() as conn:
            ...
            # ✅ NULL 비교 수정: 값이 None 이면 IS NULL, 아니면 =? 사용
            # 동적 쿼리 생성으로 NULL 처리 명확화
            conditions = [
                "domain=?",
                "task_type=?",
                "status='FAILED'",
            ...
            params = [domain, task_type]
            
            for col, val in [
                ("shard", shard), ("sc_code", sc_code), ("region", region),
                ("year", year), ("month", month), ("day", day),
                ("semester", semester), ("address", address), ("sub_key", sub_key)
            ]:
                if val is None:
