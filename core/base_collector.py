...
class NeisInfoCollector(BaseCollector):
    ...
    def _update_schools_with_diff(self, new_rows: List[dict], region_code: str, limit: Optional[int] = None) -> Tuple[int, int, int]:
        ...
        # limit이 있으면 row_meta 크기를 제한 (먼저 처리할 개수 제한)
        if limit and len(row_meta) > limit:
            # limit만큼만 남기고 나머지는 버림 (API에서 가져온 전체 데이터 중 일부만 처리)
            limited_items = list(row_meta.items())[:limit]
            row_meta = dict(limited_items)
            if self.debug_mode:
        ...
        # 첫 번째 루프: 좌표 처리 + 바로 enqueue
        for i, (sc_code, meta) in enumerate(row_meta.items(), 1):
            processed_count = i  # 실제 처리한 개수
            ...
            # 좌표 여부에 관계없이 모든 항목에 대해 enqueue 실행
            row = meta["row"]
