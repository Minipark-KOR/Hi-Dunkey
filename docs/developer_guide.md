...
│   ├── engine/collector.py      # ✅ 모든 수집기의 베이스 클래스 (v2)
│   ├── config.py                # 설정 파일 로더
│   ├── database.py              # DB 연결 공통
│   ├── schema_manager.py        # 스키마 기반 테이블 생성
│   ├── engine/collector_stats.py# 배치 처리 통계
│   ├── data/validator_data.py   # 데이터 유효성 검증
│   ├── kst_time.py              # 시간 처리
│   ├── util/manage_log.py       # 로깅
│   ├── engine/network.py        # 네트워크 요청
│   ├── engine/retry.py          # 실패 재시도 관리
│   ├── engine/shard.py          # 샤딩/범위 필터링
│   ├── school/address/          # 주소/지역 관련 모듈
│   └── ...
...
│   ├── run/                     # 운영 실행 (pipeline/vacuum/retry_worker 등)
│   ├── merge/                   # DB 병합
│   ├── analysis/                # 분석 스크립트
│   ├── misc/                    # 일회성/운영 보조
│   └── util/                    # 운영 유틸
├── constants/                   # 상수 모듈 (중앙 통제)
│   ├── map_apis.py              # API 응답 키 ↔ 내부 필드명 매핑
...
class NewCollector(CollectorEngine):
    ...
    api_context = "my_context"               # map_apis.py 에서 사용할 컨텍스트
    merge_script = "scripts/merge/new_collector.py" # 병합 스크립트 (없으면 None)
...
1. `scripts/collector/`에 새 수집기 파일을 추가합니다.
2. `python master_collectors.py --list`로 자동 인식을 확인합니다.
3. `python collector_cli.py <collector_name> ...`로 실행합니다.

```bash
# 1. 새 수집기 작성
#    예) scripts/collector/new_collector.py
...
```bash
# Flask 기반 웹 대시보드 실행
python scripts/util/dashboard.py
...
|------|---------|------------|----------|
| 원시 수집 | `collector/` | `neis_info.py` | `python scripts/collector/neis_info.py --regions B10 --year 2026 --shard odd` |
| 운영 실행 | `run/` | `pipeline.py` | `python scripts/run/pipeline.py neis_info --year 2026 --regions B10,J10` |
| DB 병합 | `merge/` | `neis_info.py` | `python scripts/merge/neis_info.py --year 2026` |
| 분석/보조 | `analysis/, misc/` | `search_schools.py` | `python scripts/analysis/search_schools.py --health` |
| 유틸리티 | `run/, util/` | `vacuum.py` | `python scripts/run/vacuum.py` |

**중요:** 모든 스크립트는 프로젝트 루트에서 `python scripts/...` 형식으로 실행하는 것을 기준으로 문서를 통일합니다.
...
`api_context`는 클래스 변수로 정의하며, `map_apis.py`에 해당 컨텍스트가 정의되어 있어야 합니다.
...
|-----------|----------|----------------|
| API 응답 키 | `constants/map_apis.py` | 모든 수집기 (`get_api_field` 사용 시) |
| DB 컬럼 | `constants/schema.py` + `migrate.py` | 모든 샤드 DB (자동 마이그레이션) |
| 샤딩 규칙 | `core/engine/shard.py` | 수집, 보강, 병합 모든 스크립트 |
| 수집 로직 | `core/engine/collector.py` | 모든 수집기 (상속받으므로 자동 적용) |
...
## 13. 병렬 실행 (`scripts/run/pipeline.py`)

```bash
# 사용법
python scripts/run/pipeline.py <collector_name> [--year YYYY] [--timeout 초] [--regions REGIONS] [--quiet]

# 예시
python scripts/run/pipeline.py neis_info --year 2026 --regions B10,J10 --quiet
```
...
- 실행 로그는 `data/logs/`에 저장되며, `run.pipeline.<collector>_<region>_<shard>.log` 및 `run.pipeline.<collector>_merge.log` 형식을 사용합니다.

## 13.1 로그 정책

- 표준 로그 디렉토리: `data/logs`
- 기본 파일명: `<domain>.log`
- 스크립트 prefix 적용: `<parent>.<domain>.log`
- 예시: `engine.meal.log`, `merge.master.log`, `misc.additional_school_info.log`
...
### 15.2. 배치 처리 통계 (`core/engine/collector_stats.py`)
...
### 15.3. 데이터 유효성 검증 (`core/data/validator_data.py`)
...
### 15.4. 주소 처리 (`core.school.address`)

```python
from core.school.address import parse_region_input, get_all_regions
...
| 변경 대상 | 수정 위치 | 자동 반영 범위 |
|-----------|----------|----------------|
| **API 응답 키** | `constants/map_apis.py` | 모든 수집기 |
| **DB 컬럼** | `constants/schema.py` + `migrate.py` | 모든 샤드 DB |
| **샤딩 규칙** | `core/engine/shard.py` | 수집/보강/병합 모든 스크립트 |
| **수집 로직** | `core/engine/collector.py` | 모든 수집기 (상속) |
...
| Engine | `{기능}.py` | `collector.py` |
| Utility | `{기능}_{역할}.py` | `schema_manager.py` |
