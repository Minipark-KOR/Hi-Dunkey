...
try:
    ...
    def get_hot_schools(limit=50):
        logger.warning("baskets.update_hot 모듈이 없어 Hot50 기능을 비활성화합니다.")
        return []

logger = build_domain_logger("run_collector", "run_collector", __file__)
...
def run_retry_worker():
    ...
    cmd = [sys.executable, "scripts/run/retry_worker.py", "--limit", "100", "--force", "--no-menu"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode != 0:
...
def run_master():
    ...
    if now_kst().day == 1:
        ...
        merge_cmd = [sys.executable, "scripts/merge/neis_info.py"]
