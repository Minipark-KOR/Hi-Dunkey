...
logger = build_domain_logger("merge_decade", "merge_decade", __file__)
...
def create_unknown_db(start: int, end: int) -> str:
    ...
    with sqlite3.connect(dest) as conn:
        for f in files[1:]:
            conn.execute(f"ATTACH DATABASE '{f}' AS src")
            conn.execute(f"INSERT OR IGNORE INTO main.{UNKNOWN_TABLE} SELECT * FROM src.{UNKNOWN_TABLE}")
            conn.execute("DETACH DATABASE src")
        conn.execute("VACUUM")
        conn.execute("ANALYZE")
        if conn.execute("PRAGMA integrity_check").fetchone()[0] != "ok":
