...
from core.data.error_classifier import classify_error
from constants.paths import GLOBAL_VOCAB_DB_PATH, NEIS_INFO_DB_PATH, SCHOOL_INFO_DB_PATH, FAILURES_DB_PATH, LOG_DIR

logger = build_domain_logger("retry_worker", "retry_worker", __file__)
...
def print_summary(success, retry, orphan, start_time, remaining):
    ...
    try:
        ...
        with open(LOG_DIR / "last_result.txt", "w", encoding="utf-8") as f:
...
def check_missing_count(neis_info_db: str):
    ...
    with sqlite3.connect(neis_info_db) as conn:
        missing = conn.execute("SELECT COUNT(*) FROM schools WHERE latitude IS NULL OR longitude IS NULL").fetchone()[0]
...
def check_failures_queue(failures_db: str):
    ...
    with sqlite3.connect(failures_db) as conn:
        rows = conn.execute("SELECT status, COUNT(*) FROM failures GROUP BY status").fetchall()
        print("\n📊 failures 큐 상태 (전체 누적)")
        for s, c in rows:
            print(f"  - {s}: {c}개")
...
def list_failed_schools(neis_info_db: str, failures_db: str):
    ...
    try:
        with sqlite3.connect(failures_db) as conn_f:
            ...
            failed_rows = {
                row['sc_code']: {'error_msg': row['error_msg'], 'retries': row['retries']}
                for row in conn_f.execute("SELECT sc_code, error_msg, retries FROM failures WHERE status='FAILED'").fetchall()
            }
        if not failed_rows:
        ...
        with sqlite3.connect(neis_info_db) as conn_s:
            ...
            rows = conn_s.execute(f"""
                SELECT sc_code, sc_name, address, jibun_address
                FROM schools WHERE sc_code IN ({placeholders})
                ORDER BY sc_name LIMIT 50
            """, list(failed_rows.keys())).fetchall()
...
def reset_expired_and_orphan(failures_db: str):
    ...
    try:
        with sqlite3.connect(failures_db) as conn:
            cur = conn.execute("""
                UPDATE failures SET status='FAILED', retries=0, resolved_at=NULL
    ...
    if confirm == 'y':
        ...
        for f in os.listdir(str(LOG_DIR)):
            if f.endswith(".log") or f == "last_result.txt":
                os.remove(os.path.join(str(LOG_DIR), f))
                deleted += 1
...
def main():
    ...
    remaining = (
        len(rm.get_all_pending_retries(limit=10000)) if args.force
        else len(rm.get_pending_retries(limit=10000, deadline=deadline))
    )
