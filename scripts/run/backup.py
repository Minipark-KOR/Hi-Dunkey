...
try:
    from core.data.backup import vacuum_into, cleanup_temp_files
...
from core.util.metrics import generate_and_save_metrics, cleanup_old_metrics
from constants.domains import DOMAIN_CONFIG, GLOBAL_DBS
from constants.paths import DATA_DIR, BACKUP_DIR, ARCHIVE_DIR, LOG_DIR, METRICS_DIR, TEMP_DIR
from dotenv import load_dotenv

BASE_DIR = Path(__file__).resolve().parent.parent.parent
load_dotenv(BASE_DIR / ".env")

from scripts.collector.neis_info import NeisInfoCollector
from scripts.collector.meal import MealCollector
from scripts.collector.timetable import AnnualFullTimetableCollector as TimetableCollector
from scripts.collector.schedule import AnnualFullScheduleCollector as ScheduleCollector
...
MERGE_DIR = BASE_DIR / "scripts" / "merge"
...
class YearlyBackup:
    def __init__(self, base_dir: str = str(DATA_DIR), backup_dir: str = str(BACKUP_DIR),
        ...
        for d in [base_dir, backup_dir, archive_dir, log_dir, str(METRICS_DIR)]:
            os.makedirs(d, exist_ok=True)

        self._setup_logger()

    def _setup_logger(self):
        self.logger = build_domain_logger("yearly_backup", "backup", __file__)
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if not issues:
                    ...
                    deleted_temp = cleanup_temp_files(str(TEMP_DIR), days=14, recursive=True)
                    if deleted_temp:
                        self.logger.info(f"🧹 temp 정리 완료: {len(deleted_temp)}개 삭제")
                    return True
    ...
    def restore_backup(self, year: int, collectors: Optional[List[str]] = None) -> bool:
        ...
        for name in targets:
            ...
            if not files:
                ...
                continue
            latest = sorted(files, reverse=True)[0]
            dst    = DOMAIN_CONFIG[name].get("db_path")
            if not dst:
                continue
    ...
    def _run_collectors(self, year: int, collectors: Optional[List[str]] = None) -> bool:
        targets = collectors if collectors else [
            n for n, i in DOMAIN_CONFIG.items() if i.get("enabled")
        ]
        for name in targets:
    ...
    def _create_total_dbs(self, year: int):
        ...
        for name, cfg in DOMAIN_CONFIG.items():
            if not cfg.get("enabled"):
                continue
            merge_script = cfg.get("merge_script")
            if not merge_script:
                self.logger.info(f"  ⏭️ {name} 병합 스크립트 없음, 건너뜀")
                continue
            script_path = MERGE_DIR / merge_script
            if not script_path.exists():
                self.logger.warning(f"  ⚠️ {name} 병합 스크립트 없음: {script_path}")
                continue
            try:
                ...
                if result.returncode != 0:
                    self.logger.error(f"  ❌ {name} 병합 실패:\n{result.stderr[:300]}")
                    raise RuntimeError(f"{name} 병합 실패")
    ...
    def _write_backups(self, year: int):
        ...
        for name, cfg in DOMAIN_CONFIG.items():
            src = cfg.get("db_path")
            if not src or not os.path.exists(src):
                self.logger.warning(f"  ⚠️ {name} DB 없음")
                continue
    ...
    def _auto_repair(self, year: int, issues: List[Dict]):
        ...
        for issue in issues:
            ...
            if t in ("count_mismatch", "sample_mismatch"):
                ...
                self.logger.warning(f"  ⚠️ 글로벌 DB 문제: {issue.get('db')} (수동 확인 필요)")
    ...
    def _repair_recollect(self, issue: Dict, year: int):
        ...
        try:
            ...
            self._create_total_dbs(year)
            backup_date = now_kst().strftime("%Y%m%d")
            src = DOMAIN_CONFIG[name]["db_path"]
            dst = os.path.join(self.backup_dir, f"{backup_date}_{name}.db")
            vacuum_into(src, dst)
