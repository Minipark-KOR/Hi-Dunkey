...
여러 region에 대해 odd/even 샤드를 병렬로 실행하며, rich를 사용한 실시간 멀티바
진행률을 표시합니다.
"""
import sys
import subprocess
...
# 도메인별 병합 스크립트 매핑 (scripts/merge/ 기준)
MERGE_SCRIPT_MAP = {
    "neis_info":   "neis_info.py",
    "school_info": "school_info.py",
    "meal":        "meal.py",
    "schedule":    None,
    "timetable":   None,
}
...
def get_merge_script(collector: str) -> Optional[str]:
    return MERGE_SCRIPT_MAP.get(collector)
...
def cleanup_old_logs(log_dir: Path, collector: str, days: int = 7):
    ...
    for log_file in log_dir.glob(f"run.pipeline.{collector}_*.log"):
        try:
...
def run_merge(collector: str, year: int, log_dir: Path, timeout: Optional[int] = None) -> bool:
    script_name = get_merge_script(collector)
    if not script_name:
        print(f"⚠️ {collector}는 병합 스크립트 없음, 건너뜀")
        return True

    merge_script_path = Path("scripts/merge") / script_name
    ...
    merge_log = log_dir / f"run.pipeline.{collector}_merge.log"
...
def run_shard(region: str, shard: str, collector: str, extra_args: List[str],
              year: int, timeout: Optional[int] = None) -> Tuple[str, str, int]:
    log_file = LOG_DIR / f"run.pipeline.{collector}_{region}_{shard}.log"
    cmd = [
...
def main():
    ...
    if failed:
        print("❌ 실패한 작업:")
        for r, s in failed:
            print(f"  - {r} {s} (로그: {LOG_DIR / f'run.pipeline.{args.collector}_{r}_{s}.log'})")
    ...
    if len(failed) == 0:
        ...
        if run_merge(args.collector, year, log_dir, timeout=args.timeout):
            ...
            print(f"👉 병합 로그: {LOG_DIR / f'run.pipeline.{args.collector}_merge.log'}")
            sys.exit(1)
    else:
