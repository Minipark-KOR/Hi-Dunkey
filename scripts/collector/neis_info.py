...
class NeisInfoCollector(CollectorEngine):
    ...
    def _transform_row(self, row: dict, region_code: str) -> dict:
        ...
        return {
            ...
            "atpt_ofcdc_sc_nm": (row.get("ATPT_OFCDC_SC_NM") or "").strip(),
            "lctn_sc_nm": (row.get("LCTN_SC_NM") or "").strip(),
            "ju_org_nm": (row.get("JU_ORG_NM") or "").strip(),
            "fond_sc_nm": (row.get("FOND_SC_NM") or "").strip(),
            "org_rdnzc": (row.get("ORG_RDNZC") or "").strip(),
            "org_rdnda": (row.get("ORG_RDNDA") or "").strip(),
            "org_faxno": (row.get("ORG_FAXNO") or "").strip(),
            "coedu_sc_nm": (row.get("COEDU_SC_NM") or "").strip(),
            "hs_sc_nm": (row.get("HS_SC_NM") or "").strip(),
            "indst_specl_ccccl_exst_yn": (row.get("INDST_SPECL_CCCCL_EXST_YN") or "").strip(),
            "hs_gnrl_busns_sc_nm": (row.get("HS_GNRL_BUSNS_SC_NM") or "").strip(),
            "spcly_purps_hs_ord_nm": (row.get("SPCLY_PURPS_HS_ORD_NM") or "").strip(),
            "ene_bfe_sehf_sc_nm": (row.get("ENE_BFE_SEHF_SC_NM") or "").strip(),
            "dght_sc_nm": (row.get("DGHT_SC_NM") or "").strip(),
            "fond_ymd": (row.get("FOND_YMD") or "").strip(),
            "foas_memrd": (row.get("FOAS_MEMRD") or "").strip(),
            "load_dtm": (row.get("LOAD_DTM") or "").strip(),
