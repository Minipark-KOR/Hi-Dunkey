...
from core.util.manage_log import resolve_domain_log_path

HEALTH_MONITOR_LOG = resolve_domain_log_path("health_monitor", __file__)
...
def monitoring_example():
    """
    크론탭에 등록할 모니터링 스크립트
    0 9 * * * cd /path/to/project && python scripts/analysis/search_schools.py --monitor
    """
    # 로그 디렉토리 생성
    os.makedirs(str(HEALTH_MONITOR_LOG.parent), exist_ok=True)
    ...
    if not healthy:
        ...
        # 파일로 저장
        with open(HEALTH_MONITOR_LOG, "a", encoding='utf-8') as f:
            f.write(log_msg)
